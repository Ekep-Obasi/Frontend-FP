...
export async function GET(req: NextRequest) {
  ...
  if (!query && !location) {
    return NextResponse.json({ error: "Provide q or location" }, { status: 400 });
  }
  ...
  try {
    if (provider === "mapbox") {
      ...
      if (!query) {
        return NextResponse.json({ error: "Mapbox provider requires q" }, { status: 400 });
      }
      const proximity = location ? `&proximity=${encodeURIComponent(location)}` : "";
    ...
    if (!apiKey) {
      return NextResponse.json({ error: "Missing GOOGLE_PLACES_API_KEY/GOOGLE_MAPS_API_KEY" }, { status: 500 });
    }
