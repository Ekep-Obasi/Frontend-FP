...
export default function Onboarding({ onComplete }: Props) {
  ...
  async function createPlan() {
    ...
    try {
      ...
      upsertPlan(plan);
      try {
        await fetch("/api/plans", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(plan) });
      } catch {}
      onComplete();
    } finally {
      setLoading(false);
        ...
        {step === 1 && (
          <div className="space-y-3">
            ...
            <div className="flex justify-end">
              <button className="mt-2 rounded-md bg-blue-600 text-white px-4 py-2 text-sm" onClick={() => setStep(2)}>Next</button>
            </div>
          </div>
        )}
        {step === 2 && (
          <div className="space-y-3">
            ...
            <input className="w-full rounded-md border px-3 py-2 bg-transparent" value={destinationHint} onChange={(e) => setDestinationHint(e.target.value)} />
            <label className="text-sm block">Interests</label>
            <input className="w-full rounded-md border px-3 py-2 bg-transparent" value={interests} onChange={(e) => setInterests(e.target.value)} />
            <div className="flex items-center justify-between mt-2">
              <button className="rounded-md border px-4 py-2 text-sm" onClick={() => setStep(1)}>Back</button>
              <button className="rounded-md bg-blue-600 text-white px-4 py-2 text-sm disabled:opacity-60" onClick={createPlan} disabled={loading}>{loading ? "Creating…" : "Create"}</button>
            </div>
          </div>
        )}
      </div>
